...
def create_bike(request: Request, bike: Bike = Body(...)):
    ...
    new_bike = request.app.database["bicicletas"].insert_one(bike)
    created_bike = request.app.database["bicicletas"].find_one(
...
def find_bike(id: str, request: Request):
    if (bike := request.app.database["bicicletas"].find_one({"_id": id})) is not None:
        return bike
    raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Bike with ID {id} not found")
